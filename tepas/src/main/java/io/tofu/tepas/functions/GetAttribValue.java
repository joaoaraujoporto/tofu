...
public class GetAttribValue extends Function<String> {

	GetAttribValue(Object ... args) {
		super(args);
	}
	...
	public String apply() {
		...
		if (i == 0 && p.getHead().getSign().equals(symbolName))
			return p.getHead().getMeaning().get(attribName).toString();
		...
		return s.getMeaning().get(attribName).toString();
	}
}
